...
Adresse de facturation : Même adresse`,
  ...
  const renderNote = (note) => {
    ...
    return (
      <div className={`note-box note-${note.type}`}>
        {icons[note.type]}
        <span>{note.text}</span>
      </div>
    );
  ...
  return (
    <div className="guide-container">
      ...
      <div className="guide-header">
        <div className="header-content">
          <div className="header-text">
            <h1 className="guide-title">GUIDE COMPLET MYSMILELAB</h1>
            <p className="guide-subtitle">Comment passer une commande ?</p>
          </div>
      ...
      <div ref={targetRef} className="guide-content">
        {/* Statistiques */}

        {/* Introduction */}
        <div className="guide-intro">
          <div className="intro-content">
            <h2> Bienvenue sur MySmileLab</h2>
            <p>
        ...
        <div className="steps-container">
          {steps.map((step) => (
            <div key={step.id} className="step-card expanded">
              ...
              <div className="step-details">
                ...
                {step.content.important && (
                  <p className="step-important">{step.content.important}</p>
                )}

                {step.content.mainPoints && (
                  <div className="main-points">
                    {step.content.mainPoints.map((point, idx) => (
                      <div key={idx} className="main-point-card">
                        <div className="main-point-header">
                ...
                {step.content.secondaryPoints && (
                  <>
                    <p className="secondary-points-title">
                      Informations complémentaires (optionnelles si déjà
                      enregistrées) :
                    </p>
                    <ul className="secondary-points-list">
                ...
                {step.content.example && (
                  <div className="example-box">
                    <h4>{step.content.example.title}</h4>
                    <pre>{step.content.example.text}</pre>
                  </div>
                )}

                {step.content.process && (
                  <p className="process-info">{step.content.process}</p>
                )}
